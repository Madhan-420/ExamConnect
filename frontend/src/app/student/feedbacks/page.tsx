...
export default function StudentFeedbacksPage() {
    ...
    const fetchFeedbacks = async () => {
        try {
            // Note: Currently, students' endpoint just submitted. But let's check if the generic feedbacks endpoint works. 
            // Wait, there is no GET /api/student/feedbacks in students.py, so we should handle if it fails gracefully.
            // Admin only view requested, but if they want to view their own complaints we'll try to fetch.
            const res = await api.get('/api/teacher/feedbacks'); // This endpoint uses user_id, it actually works for any role!
            setFeedbacks(res.data);
        } catch (err) {
            // Fallback: If it's role restricted, we just show empty list
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSubmitting(true);
        try {
            await api.post('/api/student/feedbacks', form);
            setShowModal(false);
